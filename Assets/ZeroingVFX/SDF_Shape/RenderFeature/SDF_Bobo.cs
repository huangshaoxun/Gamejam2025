...
{
    public class SDF_Bobo : ScriptableRendererFeature
    {
        class RP : ScriptableRenderPass
        {
            ...
            {
                cameraTextureDescriptor = rtDesc;
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                CommandBuffer cmd = CommandBufferPool.Get("SDF_Bobo");
                //cmd.Clear();
                ...
                material.SetMatrix(SDF_Bobo.IVP, IVP);
                material.SetInt(SDF_Bobo._MaxStep, settings.MaxStep);
                material.SetFloat(SDF_Bobo._Epsilon, settings.Epsilon);
                material.SetColor(SDF_Bobo._Color, settings.color);
                material.SetColor(SDF_Bobo._Specular, settings.Specular);
                material.SetFloat(SDF_Bobo._Reflection, settings.Reflection);
                material.SetFloat(SDF_Bobo._Warp, settings.Warp);
                material.SetFloat(SDF_Bobo._WarpScale, settings.WarpScale);
                material.SetFloat(SDF_Bobo._DepthBlend, settings.DepthBlend);
                if (settings._matcap == null)
                {
                    material.SetTexture(SDF_Bobo._MatcapTex, Texture2D.blackTexture);
                }
                else
                {
                    material.SetTexture(SDF_Bobo._MatcapTex, settings._matcap);
                }
                ...
                // CoreUtils.SetRenderTarget(cmd, renderingData.cameraData.cameraTargetDescriptor);
